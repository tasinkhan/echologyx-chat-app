    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if many:
                ...
                organizations = [Organization(**validated_data) for validated_data in serializer.validated_data]
                Organization.objects.bulk_create(organizations)
                return Response({"detail": "Organizations created successfully."}, status=status.HTTP_201_CREATED)
            else:
                Organization.objects.create(**serializer.validated_data)
                return Response({"detail": "Organization created successfully."}, status=status.HTTP_201_CREATED)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if many:
                ...
                roles = [Role(**validated_data) for validated_data in serializer.validated_data]
                Role.objects.bulk_create(roles)
                return Response({"detail": "roles created successfully."}, status=status.HTTP_201_CREATED)
            else:
