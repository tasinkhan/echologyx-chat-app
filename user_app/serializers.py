...
class UserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Create the user
        user = User.objects.create_user(role=role, organization=organization, **validated_data)
        return user

    def update(self, instance, validated_data):
        if 'password' in validated_data:
            instance.set_password(validated_data['password'])  # Hash password
            validated_data.pop('password')  # Remove password from validated data to avoid overwriting
